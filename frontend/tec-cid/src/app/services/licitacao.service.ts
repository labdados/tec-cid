...
export class LicitacaoService {
  ...
  getLicitacoesMunicipio(codUni?:any, ano?: any, tipoLic?: any){
    return this.http.get<any>(`${SERVER_URL}/licitacoes?limite=40&pagina=${1}&ano=${ano}&codUni=${codUni}&tipoLic=${tipoLic}`)
  }

  getLicitacao(idLicitacao:any) {
    return this.http.get<any>(`${SERVER_URL}/licitacoes/${idLicitacao}`)
  }

  getPropostas(idLicitacao:string) {
    return this.http.get<any>(`${SERVER_URL}/licitacoes/${idLicitacao}/propostas`).subscribe(res => {
